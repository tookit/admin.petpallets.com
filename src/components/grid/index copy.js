import 'vuetify/src/components/VDataTable/VDataTable.sass' // Components
...
import VDataTableHeader from 'vuetify/lib/components/VDataTable/VDataTableHeader' // import VVirtualTable from './VVirtualTable'

import VIcon from 'vuetify/lib/components/VIcon'
// import Row from 'vuetify/lib/components/VDataTable/Row'
import Row from './Row'
import RowGroup from 'vuetify/lib/components/VDataTable/RowGroup'
...
import Nestedable from './nestedable'

import ripple from 'vuetify/lib/directives/ripple' // Helpers
...
function searchTableItems(
  ...
  return items.filter((item) => {
    ...
    const matchesColumnFilters = headersWithCustomFilters.every(
      filterFn(item, search, defaultFilter)
...
/* @vue/component */
export default mixins(VDataIterator, Loadable, Nestedable).extend({
  name: 'v-grid',
  ...
  computed: {
    ...
    headersWithCustomFilters() {
      return this.headers.filter(
        (header) =>
          header.filter &&
          (!header.hasOwnProperty('filterable') || header.filterable === true)
      )
    },
  ...
  methods: {
    ...
    createItemProps(item, index) {
      const props = VDataIterator.options.methods.createItemProps.call(
        this,
        item,
        index
      )
    ...
    genHeaders(props) {
      ...
      const children = [
        getSlot(this, 'header', { ...data, isMobile: this.isMobile })
      ]

      if (!this.hideDefaultHeader) {
        const scopedSlots = getPrefixedScopedSlots('header.', this.$scopedSlots)
        children.push(
          this.$createElement(VDataTableHeader, { ...data, scopedSlots })
        )
      }
    ...
    genDefaultGroupedRow(group, items, props) {
      ...
      const toggleFn = () =>
        this.$set(this.openCache, group, !this.openCache[group])
    ...
    genDefaultRows(items, props) {
      return this.$scopedSlots['expanded-item']
        ...
        : items.map((item, index) => {
            const rows = this.genDefaultSimpleRow(item, index)
            return rows
          })
    },
    ...
    genDefaultScopedSlot(props) {
      ...
      return this.$createElement(
        ...
        [
          this.proxySlot(
            'top',
            getSlot(this, 'top', { ...props, isMobile: this.isMobile }, true)
          ),
  ...
  render() {
    return this.$createElement(VData, {
      ...
      on: {
        ...
        pagination: (v, old) =>
          !deepEqual(v, old) && this.$emit('pagination', v),
        'current-items': (v) => {
